(function (factory) {
    ...
    const UNIT_DISPLAYS = ['narrow', 'long', 'short'];
    ...
    function produceSignPattern(positivePattern, negativePattern = '') {
        ...
        const alwaysPositivePattern = positivePattern.includes(`{${InternalSlotToken.plusSign}}`)
            ? positivePattern
    ...
    function extractSignPattern(pattern) {
        ...
        const [positivePattern, negativePattern] = patterns.map(p => p
     ...
     * @param pattern
     */
    function extractCompactSymbol(pattern, slotToken = InternalSlotToken.compactSymbol) {
    ...
    function extractDecimalPattern(d, numberingSystem) {
        const compactShortSignPattern = extractSignPattern(extractCompactSymbol(d.decimal[numberingSystem].short[SPECIAL_NUMBER_HACK].other));
        const compactLongSignPattern = extractSignPattern(extractCompactSymbol(d.decimal[numberingSystem].long[SPECIAL_NUMBER_HACK].other, InternalSlotToken.compactName));
    ...
    function extractPercentPattern(d, numberingSystem) {
        ...
        const scientificPercentSignPattern = extractSignPattern(d.percent[numberingSystem].replace(NUMBER_PATTERN, SCIENTIFIC_SLOT));
        return SIGN_DISPLAYS.reduce((all, k) => {
            all[k] = {
    ...
    function extractStandardCurrencyPattern(currencyStandardPattern, currencyUnitPattern, currencyShortPattern, decimalShortPattern, decimalLongPattern, currencyDisplay) {
        let standardPattern;
        let scientificPattern;
        let compactShortPattern;
        let compactLongPattern;
        ...
        if (currencyDisplay === 'name') {
            standardPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, (_, p) => p === '0' ? DUMMY_PATTERN : `{${InternalSlotToken.currencyName}}`);
            scientificPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, (_, p) => p === '0' ? SCIENTIFIC_SLOT : `{${InternalSlotToken.currencyName}}`);
            compactShortPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, (_, p) => p === '0'
                ? extractCompactSymbol(decimalShortPattern)
                : `{${InternalSlotToken.currencyName}}`);
            compactLongPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, (_, p) => p === '0'
        ...
        return SIGN_DISPLAYS.reduce((all, signDisplay) => {
            all[signDisplay] = {
    ...
    function extractCurrencyPatternForCurrency(d, c, currency, numberingSystem) {
        var _a;
        const insertBetween = d.currency[numberingSystem].currencySpacing.beforeInsertBetween;
        ...
        const currencyShortPattern = ((_a = d.currency[numberingSystem].short) === null || _a === void 0 ? void 0 : _a[1000].other) || '';
    ...
    function generateUnitPatternPayload(unitPatternStr, display, compactShortDecimalPattern, compactLongDecimalPattern) {
        const unitToken = display === 'long'
            ? InternalSlotToken.unitName
            : display === 'short'
                ? InternalSlotToken.unitSymbol
                : InternalSlotToken.unitNarrowSymbol;
        const standardUnitPatternStr = unitPatternStr.replace(STANDARD_PATTERN_REGEX, (_, p) => (p === '0' ? DUMMY_PATTERN : `{${unitToken}}`));
        const scientificUnitPatternStr = unitPatternStr.replace(STANDARD_PATTERN_REGEX, (_, p) => (p === '0' ? SCIENTIFIC_SLOT : `{${unitToken}}`));
        const compactShortUnitPatternStr = unitPatternStr.replace(STANDARD_PATTERN_REGEX, (_, p) => p === '0'
            ? extractCompactSymbol(compactShortDecimalPattern)
            : `{${unitToken}}`);
        const compactLongUnitPatternStr = unitPatternStr.replace(STANDARD_PATTERN_REGEX, (_, p) => p === '0'
    ...
    function extractPatterns(units, currencies, numbers, numberingSystem, unit, currency) {
        return {
            ...
            unit: unit
                ? {
                    [unit]: extractUnitPatternForUnit(numbers, units, unit, numberingSystem),
                }
                : {},
            currency: currency
                ? {
                    [currency]: extractCurrencyPatternForCurrency(numbers, currencies, currency, numberingSystem),
                }
                : {},
        };
    }
    ...
    function initializeNumberFormat(nf, locales, opts) {
        ...
        const r = createResolveLocale(UnifiedNumberFormat.getDefaultLocale)(getInternalSlot(__INTERNAL_SLOT_MAP__, nf, 'availableLocales'), requestedLocales, opt, getInternalSlot(__INTERNAL_SLOT_MAP__, nf, 'relevantExtensionKeys'), localeData);
    ...
    class UnifiedNumberFormat {
        ...
        constructor(locales, options) {
            ...
            const newTarget = this && this instanceof UnifiedNumberFormat ? this.constructor : void 0;
            if (!newTarget) {
            ...
            const { localeData: { [getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'locale')]: ildData, }, } = UnifiedNumberFormat;
                    ...
                    switch (p) {
                        case InternalSlotToken.number: {
                            if (isNaN(formattedX)) {
                                ...
                                const { numberingSystem: nu, useGrouping } = getMultiInternalSlots(__INTERNAL_SLOT_MAP__, this, 'numberingSystem', 'useGrouping');
                        ...
                        case InternalSlotToken.compactName:
                            const compactData = ild.decimal[p === 'compactName' ? 'compactLong' : 'compactShort'];
                            if (compactData) {
                                results.push({
                                    type: 'compact',
                        ...
                        case InternalSlotToken.unitName: {
                            ...
                            if (style === 'unit') {
                                ...
                                const mu = selectPlural(this.pl, formattedX, unitSymbols[p] || unitSymbols[InternalSlotToken.unitSymbol]);
