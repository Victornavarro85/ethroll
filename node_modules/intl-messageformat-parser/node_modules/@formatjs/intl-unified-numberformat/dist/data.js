...
function produceSignPattern(positivePattern, negativePattern) {
    ...
    if (!negativePattern) {
        negativePattern = "{" + intl_utils_1.InternalSlotToken.minusSign + "}" + positivePattern;
    }
    var zeroPattern = positivePattern;
    var alwaysPositivePattern = positivePattern.includes("{" + intl_utils_1.InternalSlotToken.plusSign + "}")
        ? positivePattern
        : "{" + intl_utils_1.InternalSlotToken.plusSign + "}" + positivePattern;
    return {
        always: {
...
function extractSignPattern(pattern) {
    ...
    var _a = patterns.map(function (p) {
        return p
            .replace(NUMBER_PATTERN, "{" + intl_utils_1.InternalSlotToken.number + "}")
            .replace('+', "{" + intl_utils_1.InternalSlotToken.plusSign + "}")
            .replace('-', "{" + intl_utils_1.InternalSlotToken.minusSign + "}")
            .replace('%', "{" + intl_utils_1.InternalSlotToken.percentSign + "}");
...
function extractDecimalPattern(d, numberingSystem) {
    ...
    var compactLongSignPattern = extractSignPattern(extractCompactSymbol(d.decimal[numberingSystem].long[SPECIAL_NUMBER_HACK].other, intl_utils_1.InternalSlotToken.compactName));
...
function extractStandardCurrencyPattern(currencyStandardPattern, currencyUnitPattern, currencyShortPattern, decimalShortPattern, decimalLongPattern, currencyDisplay) {
    ...
    if (currencyDisplay === 'name') {
        standardPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, function (_, p) {
            return p === '0' ? DUMMY_PATTERN : "{" + intl_utils_1.InternalSlotToken.currencyName + "}";
        });
        scientificPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, function (_, p) {
            return p === '0' ? SCIENTIFIC_SLOT : "{" + intl_utils_1.InternalSlotToken.currencyName + "}";
        });
        compactShortPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, function (_, p) {
            return p === '0'
                ...
                : "{" + intl_utils_1.InternalSlotToken.currencyName + "}";
        });
        compactLongPattern = currencyUnitPattern.replace(STANDARD_PATTERN_REGEX, function (_, p) {
            return p === '0'
                ? extractCompactSymbol(decimalLongPattern, intl_utils_1.InternalSlotToken.compactName)
                : "{" + intl_utils_1.InternalSlotToken.currencyName + "}";
        });
    }
    ...
    return CURRENCY_DISPLAYS.reduce(function (all, currencyDisplay) {
        if (currencyDisplay === 'name') {
            ...
            var currencyToken = currencyDisplay === 'code'
                ? intl_utils_1.InternalSlotToken.currencyCode
                : currencyDisplay === 'symbol'
                    ? intl_utils_1.InternalSlotToken.currencySymbol
                    : intl_utils_1.InternalSlotToken.currencyNarrowSymbol;
...
function generateUnitPatternPayload(unitPatternStr, display, compactShortDecimalPattern, compactLongDecimalPattern) {
    var unitToken = display === 'long'
        ? intl_utils_1.InternalSlotToken.unitName
        : display === 'short'
            ? intl_utils_1.InternalSlotToken.unitSymbol
            : intl_utils_1.InternalSlotToken.unitNarrowSymbol;
    ...
    var compactLongUnitPatternStr = unitPatternStr.replace(STANDARD_PATTERN_REGEX, function (_, p) {
        return p === '0'
            ? extractCompactSymbol(compactLongDecimalPattern, intl_utils_1.InternalSlotToken.compactName)
            : "{" + unitToken + "}";
    });
