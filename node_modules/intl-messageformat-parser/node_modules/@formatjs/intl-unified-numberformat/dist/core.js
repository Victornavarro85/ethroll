...
function initializeNumberFormat(nf, locales, opts) {
    ...
    var r = intl_utils_1.createResolveLocale(UnifiedNumberFormat.getDefaultLocale)(intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, nf, 'availableLocales'), requestedLocales, opt, intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, nf, 'relevantExtensionKeys'), localeData);
    ...
    intl_utils_1.setNumberFormatDigitOptions(__INTERNAL_SLOT_MAP__, nf, options, mnfdDefault, mxfdDefault, notation);
...
var UnifiedNumberFormat = /** @class */ (function () {
    ...
    function UnifiedNumberFormat(locales, options) {
        ...
        var _a = intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'locale'), ildData = UnifiedNumberFormat.localeData[_a];
        this.pl = new Intl.PluralRules(locales);
        intl_utils_1.setMultiInternalSlots(__INTERNAL_SLOT_MAP__, this, {
            patterns: data_1.extractPatterns(ildData.units, ildData.currencies, ildData.numbers, intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'numberingSystem'), intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'unit'), intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'currency')),
        });
    ...
    UnifiedNumberFormat.prototype.formatToParts = function (x) {
        ...
        var exponent = 0;
        var ild = intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'ild');
        var n;
        x = toNumeric(x);
        var formattedX = x;
        if (isNaN(x)) {
            ...
            if (intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'style') === 'percent') {
                formattedX *= 100;
            }
                ...
                switch (p) {
                    case intl_utils_1.InternalSlotToken.number: {
                        if (isNaN(formattedX)) {
                            ...
                            var _a = intl_utils_1.getMultiInternalSlots(__INTERNAL_SLOT_MAP__, this, 'numberingSystem', 'useGrouping'), nu = _a.numberingSystem, useGrouping = _a.useGrouping;
                            if (nu && data_1.ILND.hasOwnProperty(nu)) {
                                // Replace digits
                                var replacementTable = data_1.ILND[nu];
                                var replacedDigits = '';
                    ...
                    case intl_utils_1.InternalSlotToken.unitName: {
                        var style = intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'style');
                        if (style === 'unit') {
                            var unit = intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'unit');
                            var unitSymbols = ild.unitSymbols[unit];
                            var mu = selectPlural(this.pl, formattedX, unitSymbols[p] || unitSymbols[intl_utils_1.InternalSlotToken.unitSymbol]);
                    ...
                    case intl_utils_1.InternalSlotToken.currencyName: {
                        var currency = intl_utils_1.getInternalSlot(__INTERNAL_SLOT_MAP__, this, 'currency');
                        var cd = void 0;
                        if (p === intl_utils_1.InternalSlotToken.currencyCode) {
                            cd = currency;
                        }
                        else if (p === intl_utils_1.InternalSlotToken.currencyName) {
...
function computeExponent(numberFormat, x) {
    ...
    var magnitude = utils_1.logBase10(x);
    ...
    var newMagnitude = utils_1.logBase10(x);
    ...
    switch (notation) {
        ...
        case 'compact': {
            ...
            if (num > thresholds[thresholds.length - 1]) {
                return utils_1.logBase10(+thresholds[thresholds.length - 1]);
            }
            var i = thresholds.indexOf(num);
            for (; i > 0 &&
                thresholdMap[thresholds[i - 1]].other === thresholdMap[num].other; i--)
                ;
            return utils_1.logBase10(+thresholds[i]);
        }
    }
}
/**
